...
//		NSLog(@"Value (%f,%f), Screen (%f,%f)", point.x, point.y, screenPoint.x, screenPoint.y);
    ...
    seriesLayer.frame = self.bounds;
    seriesLayer.path = seriesPath.CGPath;
    CAShapeLayer *newLayer = [self styledLayerForSeries:2];
...
- (CAShapeLayer *)styledLayerForSeries:(NSUInteger)series {
    ...
    if ([self.delegate respondsToSelector:@selector(lineGraph:widthForSeries:)]) {
        layer.lineWidth = [self.delegate lineGraph:self widthForSeries:series];
    }
    else {
        layer.lineWidth = 2.f;
    }
...
- (void)animateGraph {
    ...
    for (id seriesKey in [self.seriesLayers allKeys]) {
        ...
        NSTimeInterval animationDuration = [self.delegate lineGraph:self animationDurationForSeries:[seriesKey unsignedIntegerValue]];
